...
func buildFixture(t *testing.T, directory, packagePath string) string {
	...
	// These fixtures exercise this checkout through a local replace, including
	// when the release gate runs on a clean tag. Released-generator behavior is
	// tested separately with an explicit version.
	runGo(t, directory, "build", "-buildvcs=false", "-o", binary, packagePath)
	return binary
}
